...
########### load data #############
addrs = sc.textFile("data/addrs.csv").map(lambda l: l.split(",")).map(lambda x: (str(x[0]),str(x[1]), float(x[2])))
# addrs: [payer_addr, payee_addr, amount, tx_hash]

node = sc.textFile("data/user_node2.csv").map(lambda l: l.split(",")).map(lambda x: (str(x[0]),int(x[1])))
...
with open("output/joint_total_node.csv", "w") as f1:
...
with open("output/joint_total_edge.csv", "w") as f2:
